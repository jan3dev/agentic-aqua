...
| `wapupay.py` | WapuPay Argentine direct-fiat payments, called **directly** with `X-API-Key`; never both key + Bearer. `exchange_rates` is public. create-order returns a Liquid USDT funding address. Rail pinned USDT/LIQUID. API key resolves env `WAPUPAY_API_KEY` → stored `api_key.json` (`_require_api_key`). `wapupay_provision_account` gets/rotates the key via the AQUA backend (`POST /api/v1/wapupay/account/`, Bearer JWT, host `AQUA_BACKEND_API_URL`) and stores it; non-rotating by default. The `aqua_*` tools are a decoupled AQUA-account email-OTP login (JWT) against Ankara. Dark-launched (disabled-by-default). | `WapuPayClient`, `WapuPayManager`, `WapuPaySession`, `WapuPayApiKey`, `WapuPayOrder` |

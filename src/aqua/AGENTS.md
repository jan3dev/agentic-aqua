...
| `ankara.py` | JAN3 / AQUA / Ankara backend (host `ANKARA_API_URL`). Two surfaces: Lightning receive (L-BTC) swaps, **and** AQUA-account auth — email-OTP→JWT login (the `aqua_*` tools), session persistence (`~/.aqua/jan3/session.json`), and the WapuPay-key provisioning call (`POST /api/v1/wapupay/account/`, Bearer JWT). Shared HTTP/PII helpers (`_redact`/`_scrub_text`/`_mask`/`_extract_error_message`, `_SENSITIVE_LOG_FIELDS`) live here; `wapupay.py` imports them (one-way dep — ankara never imports wapupay). Future JAN3 account features belong here. | `AnkaraClient`, `AnkaraSwapInfo`, `JAN3AuthClient`, `JAN3AccountManager`, `JAN3Session` |
...
| `wapupay.py` | WapuPay Argentine direct-fiat calls, made directly with `X-API-Key`. `exchange_rates` is public. create-order returns a Liquid USDT funding address. Rail pinned USDT/LIQUID. API key resolves env `WAPUPAY_API_KEY` → stored `api_key.json` (`_require_api_key`). `wapupay_provision_account` delegates the AQUA-backend call to `JAN3AccountManager.provision_wapupay_token()` (in `ankara.py`), then stores the key; no-op when one already exists (env or stored) since the backend rotates on every call. AQUA-account auth (the `aqua_*` login / session) lives in `ankara.py` (injected as `self.jan3`). Dark-launched (disabled-by-default). | `WapuPayClient`, `WapuPayManager`, `WapuPayApiKey`, `WapuPayOrder` |

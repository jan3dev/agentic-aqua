...
| `pix.py` | PIX→DePix on-ramp through Ankara only. Reuses JAN3 JWT sessions, orchestrates Noviuz Hosted KYC session/confirm, creates Ankara deposits, and persists local status metadata. Never calls Eulen/Noviuz APIs directly or handles identity PII. | `EulenClient`, `PixManager`, `PixSwap` |

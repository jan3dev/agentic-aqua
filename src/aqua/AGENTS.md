...
| `lightning.py` | Unified Lightning send (Indra/Boltz) + receive (Ankara) manager. Maps provider statuses → `pending`/`processing`/`completed`/`failed`. | `LightningManager`, `LightningSwap` |

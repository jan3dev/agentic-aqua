...
| `wapupay.py` | WapuPay Argentine direct-fiat payments, called **directly** with `X-API-Key`; never both key + Bearer. `exchange_rates` is public. create-order returns a Liquid USDT funding address. Rail pinned USDT/LIQUID. API key resolves env `WAPUPAY_API_KEY` → stored `api_key.json` (`_require_api_key`). `wapupay_provision_account` gets the key via the AQUA backend (`POST /api/v1/wapupay/account/`, Bearer JWT, host `ANKARA_API_URL`) and stores it. The backend rotates on every call (always issues a fresh key, invalidates the previous), so the tool only calls it when no key is configured yet; if one already exists (env or stored) it is a no-op. The `aqua_*` tools are a decoupled AQUA-account email-OTP login (JWT) against Ankara. Dark-launched (disabled-by-default). | `WapuPayClient`, `WapuPayManager`, `WapuPaySession`, `WapuPayApiKey`, `WapuPayOrder` |

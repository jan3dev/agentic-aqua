...
| `boltz_refund.py` | Recovery of a failed send swap's L-BTC lockup: rebuilds the taproot swap tree, then spends it cooperatively (MuSig2 key path) or unilaterally (refund leaf, after the timeout). See `docs/REFUND.md`. | `refund_submarine_swap`, `build_swap_tree`, `elements_taproot_sighash` |

...
def test_all_tools_enabled_by_default():
    ...
    SideSwap graduated from opt-in to enabled-by-default (migration
    `0001_enable_sideswap_drop_pix`); the PIX tools were removed outright. New
    opt-in tools would be added to `_SHIPPED_DISABLED` in `features.py`.
    """
